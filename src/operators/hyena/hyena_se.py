...
# Minimal implementation of Hyena-SE operator (Sequence mixer) proposed in https://arxiv.org/abs/2503.01868?.
...
class Hyena_SE(nn.Module):
    def __init__(
        ...
        r"""
        Hyena-SE operator described in the paper https://arxiv.org/abs/2506.05340
    ...
    def _initialize_inner_filter_as_identity(self):
        with torch.no_grad():
            ...
            self.inner_filter.weight[:, 0, self.short_filter_order-1] = 1.0
    ...
    def forward(self, u):
        ...
        v = k * v
        v = self.inner_filter(v) # Inner filter (Explicit)
        y = q * v
        y = y.transpose(1, 2)
        y = self.out_proj(y)
        return y
...
def test_hyena_se():
    ...
    instance = Hyena_SE(1152, 256).to(dtype).to(device)
    print(instance)
    num_params = count_parameters(instance)
    print(f"Total parameters: {num_params:,}")

    # Test the operator by simply passing an input of size (1, seq_len, embedding_dim)
    seq_len = 256
    embedding_dim = 1152
    ...
    path = '/results/scion_training/8k/short_hyena_se/block_0/mae/scaled_pred=False/seed=0/checkpoints/best_train.pt'
