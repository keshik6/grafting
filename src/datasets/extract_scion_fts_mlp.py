...
"""
This code extracts all intermediate features for mlp operators in DiT for training Scions.
With a single A100 GPU, it takes about 150 mins to extract all intermediate features for 50k. 
"""
#################################################################################
...
def get_activations_by_indices(model, input_data, layer_indices):
    ...
    for layer_path in layer_indices:
        ...
        modulation_layer_path = layer_path.replace(".fc1", ".fc2") # This is the modulation layer name
...
def main(args): 
    ...
    if accelerator.is_main_process:
        scion_fts_path = os.path.join(args.feature_path, f'scion_fts_mlp/{data_split}')
        os.makedirs(scion_fts_path, exist_ok=True)
        [os.makedirs(f'{scion_fts_path}/block_{i}_mlp', exist_ok=True) for i in args.dit_block_indexes]
        start_shard_index = extract_shard_index(args.tar_str_indexes)
        print(f'Start shard index = {start_shard_index}')
        sinks = [ShardWriter(f'{scion_fts_path}/block_{i}_mlp/split_%03d.tar', maxcount=args.shard_size, start_shard=start_shard_index, maxsize=10e9) for i in args.dit_block_indexes]
    
    # Save the initialized checkpoint to track weights
    if accelerator.is_main_process:
    ...
    image_counts = {}
    block_names = [f'blocks[{i}].mlp.fc1' for i in args.dit_block_indexes]
    ...
    for index, (x, y, class_names, human_readable_class_names, image_paths) in enumerate(loader):
        ...
        if accelerator.is_main_process:
            ...
            # Looping through keys (do not loop through .fc2 as you need to extract them and save them in the same sample)
            for layer_n in all_activations:
                ...
                layer_n_adaln_name = layer_n.replace('.fc1', '.fc2')
                ...
                for i in range(all_activations[layer_n]['input'].size(0)):
                    ...
                    # Fix: Please clone when creating memory buffers.
                    ...
                    sample = {
                        ...
                        "fc1_in_fts.pth": in_fts.getvalue(), # use .pth extension for saving tensors.
                        "fc1_out_fts.pth": out_fts.getvalue(), # use .pth extension for saving tensors.
                        ...
                        "fc2_in_fts.pth": adaln_in_fts.getvalue(), # use .pth extension for saving tensors.
                        "fc2_out_fts.pth": adaln_out_fts.getvalue(), # use .pth extension for saving tensors.
